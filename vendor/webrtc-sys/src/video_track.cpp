...
#define VISIO_LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, "VISIO_WEBRTC", __VA_ARGS__)
#else
#define VISIO_LOGD(...)
#endif
...
void VideoTrack::add_sink(const std::shared_ptr<NativeVideoSink>& sink) const {
  webrtc::MutexLock lock(&mutex_);
  VISIO_LOGD("add_sink: track=%p sink=%p enabled=%d state=%d sinks_count=%zu",
             (void*)track(), sink.get(), track()->enabled(),
             static_cast<int>(track()->state()), sinks_.size());
  track()->AddOrUpdateSink(sink.get(),
                           webrtc::VideoSinkWants());  // TODO(theomonnom): Expose
                                                    // VideoSinkWants to Rust?
  sinks_.push_back(sink);
  VISIO_LOGD("add_sink: done, sinks_count=%zu", sinks_.size());
}
...
void NativeVideoSink::OnFrame(const webrtc::VideoFrame& frame) {
  VISIO_LOGD("OnFrame: %dx%d rotation=%d ts=%lld",
...
void NativeVideoSink::OnDiscardedFrame() {
  VISIO_LOGD("OnDiscardedFrame called");
